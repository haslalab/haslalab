...
class BenchCanvas(tk.Canvas):
    ...
    def _plate_views(self, x0, y0):
        ...
        self.create_text(x0, y0 + 42, text="PMMA, front",
        ...
        self.create_text(ex, ey + 42, text="PMMA surface, 0.25 mm",
                         fill="#5c6169", font=("TkDefaultFont", 7))
        self.create_text(x0 - 44, ey, text="life\nsize",
    ...
    def redraw(self):
        ...
        self.create_text(self.W - 28, yb + 26, text="optical bench, cm",
        ...
        self.create_rectangle(xl - 34, yl - 6, xl - 26, yl + 6,
                              fill="#2f343a", outline="")
        self.create_text(xl + 2, yl - 22, text="laser", fill="#4a5059",
        ...
        self.create_rectangle(xs - 4, ys_ - 5, xs + 4, ys_ + 5,
                              fill="#e9e7e2", outline="")
        self.create_text(xs, ys_ - 104, text="screen (hole)",
        ...
        self.create_text(xt, yt - 72, text="PMMA target", fill="#4d707a",
                         font=("TkDefaultFont", 8))
        ...
        self.create_text(12, 14, text="drag the laser, the screen and the target along the rail, "
                         "and up and down to set their heights", anchor="w", fill="#8b8f96",
                         font=("TkDefaultFont", 8))
...
class ControlCanvas(tk.Canvas):
    ...
    def redraw(self):
        ...
        self.create_text(30, 44, text="CONTROL BOX", anchor="w",
        ...
        self.create_text(self.SX + self.SW / 2, self.SY - 10, text="POWER",
        ...
        for k, (lab, val, unit) in enumerate((("DC mA", i, "mA"),
            ...
            self.create_oval(X + 40, 120, X + 64, 144, fill="#8d3733",
                             outline="#5c1f1c")
            self.create_text(X + 52, 176, text="multimeter", fill="#f3dedd",
                             font=("TkDefaultFont", 7))
...
class WiringPage(tk.Canvas):
    ...
    def redraw(self):
        ...
        self.create_text(330, 364, text="POWER", fill=self.NAVY,
                         font=("TkDefaultFont", 7, "bold"))
        self.create_text(320, 440, text="\u25c4 DC 12V", anchor="w",
                         fill=self.NAVY, font=("TkDefaultFont", 8, "bold"))
        self.create_text(360, 490, text="CONTROL BOX", fill=self.NAVY,
                         font=("TkDefaultFont", 8, "bold"))

        self.create_rectangle(96, 446, 180, 494, fill="#3b3f44",
                              outline="#1b1d1f", width=2)
        self.create_rectangle(104, 454, 148, 466, fill="#d8dade", outline="")
        self.create_text(138, 512, text="adaptor  7", fill="#c9ccd1",
                         font=("TkDefaultFont", 8))

        self._meter(452, 60, "DC current",
                    "%.1f" % i if i is not None else "", "mA", st["amp"])
        self._meter(692, 60, "DC voltage",
        ...
        self.create_rectangle(700, 356, 730, 386, fill="#2b2e32",
                              outline="#1b1d1f")
        self.create_text(630, 434, text="laser diode head  1", fill="#c9ccd1",
                         font=("TkDefaultFont", 8))
        if self.app.emitting():
        ...
        for k, d in self.T.items():
            ...
            if d["lab"]:
                self.create_text(d["x"] + 16, d["y"], text=d["lab"],
                                 ...
                                 font=("TkDefaultFont", 9, "bold"))
        if st["shorted"]:
            self.create_text(505, 536, text="the ammeter is across the laser and has shorted it out",
                             fill="#ff9090", font=("TkDefaultFont", 9, "bold"))
        self.create_text(20, 24, anchor="w", fill="#aeb4ba",
                         font=("TkDefaultFont", 8),
                         text="Click a terminal, then click the one it goes to.  Click a "
                              "lead to pull it out, right-click to cancel.")
...
class App(tk.Tk):
    def __init__(self):
        ...
        top = ttk.Frame(self)
        top.pack(fill="x")
        ttk.Label(top, text="  APhO 2023  \u2013  Experimental Problem 2  \u2013  "
                        "interference from a thermally deformed surface", font=("TkDefaultFont", 11, "bold")
                  ).pack(side="left", pady=5)
        self.lbl_seed = ttk.Label(top, text="PMMA plate %d" % self.seed)
        ...
        self.nb.add(page_bench, text="  optical bench and screen  ")
        self.nb.add(page_wire, text="  cabling  ")
        ...
        st = ttk.LabelFrame(left, text="adjustable stand")
        row = ttk.Frame(st)
        row.pack(padx=6, pady=4, anchor="w")
        ttk.Label(row, text="slide the plate:").pack(side="left",
                                                     padx=(0, 6))
        ...
        note = ttk.LabelFrame(left, text="Notes")
        note.pack(fill="x", padx=6, pady=(0, 6))
        ttk.Label(note, justify="left", wraplength=500, text=(
            "Turn the current knob with the mouse wheel over it, or click "
        ...
        self.lbl_cur = ttk.Label(
            ...
            text="Drag the 30 cm plastic ruler onto the pattern and read the "
                 "fringes off its own millimetre scale.  Roll the wheel "
                 "over it to swing it round, so it can be laid across a "
                 "diameter.  The fringes show through it, a little "
                 "dimmed.")
    ...
    def _spot_label(self):
        k = self.model.spot_key
        self.lbl_spot.config(text="spot %+d, %+d" % (k[0], k[1]))
        self.lbl_seed.config(text="PMMA plate %d" % self.seed)
