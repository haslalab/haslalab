...
class App:
    ...
    def set_power(self, on):
        ...
        self.lbl_sel.config(text="wheel acts on:  -  (click a dial first)")
        if self.power:
    ...
    def release(self):
        ...
        if msg is None and not self.auto_done:
            msg = ("the scope is not locked to anything yet: press Auto "
                   "Setup once to set the timebase and the level.")
        if msg:
    ...
    def _wire_hint(self):
        ...
        f = self.wiring.fault()
        if f == "open":
            return ("nothing is feeding CH1: check the source lead and "
                    "the CH1/2 lead on the cabling page.")
        if f == "wrongch":
            return "the box is on CH2 but you are looking at CH1."
        if f == "float":
            return ("the ground lead is missing, so the input is "
                    "floating: what you see is the mains pickup and drift "
                    "the cable has collected, not your signal.  Join the "
                    "GND of the box to the scope ground on the cabling "
                    "page.")
        if not self.auto_done:
            return ("the scope is not locked to anything yet: after you "
                    "change the leads, press Auto Setup once to set the "
                    "timebase and the level.")
        return ""
    ...
    def _free_run(self, floating):
        ...
        self.txt_sa.set_text("Sa %s\nCurr %s" % (fs_label(fs),
                                                 mem_label(nn)))
        self.lbl_acq.config(text="acquired at %s/div\n%s , %s"
