...
# The trajectory solvers run in background threads.  A pure Python loop
# holds the GIL for a whole switch interval at a time, and with the 5 ms
# default that is long enough for the animation to visibly stutter every
# time a solve is running.  Ask for a much finer slice, and the workers
# additionally hand the interpreter over by hand (see Bench.simulate).
try:
...
try:
    import fastcore as _fast
except Exception:
    _fast = None

# Set APHO_NO_FASTCORE=1 to force the readable Python loop everywhere,
# which is the reference the compiled one was checked against.
if os.environ.get("APHO_NO_FASTCORE"):
    _fast = None
import random

import numpy as np

# a Korean face for the few labels that matplotlib draws
import matplotlib as _mpl
_mpl.rcParams["font.family"] = ["Malgun Gothic", "AppleGothic",
                                "NanumGothic", "Noto Sans CJK KR",
                                "DejaVu Sans"]
_mpl.rcParams["axes.unicode_minus"] = False
# the trace is a few thousand points of peak-detect envelope: let Agg drop
# the vertices that land on a pixel it has already painted
...
class Bench:
    ...
    def clone(self):
        """
        A copy for the background solver.  deepcopy would drag the whole
        previous result array along with it, which is megabytes and is
        exactly the sort of thing that makes a release feel heavy.
        """
        b = object.__new__(Bench)
        b.__dict__.update(self.__dict__)
        b.rng = random.Random()
        b.rng.setstate(self.rng.getstate())
        b.ball = list(self.ball)
        b.l = list(self.l)
        b.xh = list(self.xh)
        b.result = None
        return b
    ...
    def config_name(self):
        ...
        if dz > 0.6e-3:
            return ("Fig 4.a    dz = %.1f mm, 와이어 거의 평행   (Part C / D)" % (dz * 1e3))
        if gap > 0.4e-3:
            return ("Fig 4.b    간격  dx - 2R = %.1f mm   (Part C / D)" % (gap * 1e3))
        if gap < -0.2e-3:
            return ("Fig 4.c    행어가 %.1f mm 접촉, 와이어 기울기 %.2f deg   (Part C / D)"
                    % (-gap * 1e3, math.degrees(lean)))
        return ("Fig 4.d    접촉, 와이어 평행, 행어 분리   (Part C / D)")
    ...
    def simulate(self, tmax=10.0, frame_dt=0.004, max_steps=2_000_000,
                 init=None, should_stop=None, throttle=False):
        """
        ...
        # ---- the compiled path -------------------------------------
        # With one pendulum the motion is a small self-contained ODE and
        # it has to be followed for minutes, so it runs through a
        # compiled loop that is line for line the same as the one below.
        # Two balls keep the readable version: that is the physics of
        # Parts C and D and it is worth having in one obvious piece.
        if (not two and _fast is not None and _fast.HAVE_NUMBA
                and should_stop is None and tmax > 0.0):
            ft, fx, fz, et, eup, nst, px, pz, vx, vz = _fast._run_single(
                float(tmax), 8.0e-4, float(frame_dt), int(max_steps),
                float(p1[0]), float(p1[1]), float(v1[0]), float(v1[1]),
                float(P1[0]), float(l1), float(m1), float(gg), float(air),
                float(CQ1), float(R1), float(self.x_gate), float(z_gate),
                float(amp_w), float(f_w))
            n = len(ft)
            pp = np.zeros((n, 6))
            pp[:, 0] = fx
            pp[:, 1] = fz
            ...
            for k in range(len(et)):
                if eup[k] and start is None:
                    start = float(et[k])
                elif not eup[k] and start is not None:
                    gate.append((start, float(et[k])))
                    start = None
            self.result = dict(t=ft, p=pp, gate=gate, contact=[], segments=[],
                               psi=None, f_vib=f_w, th_eq=th_eq, steps=nst)
            return self.result
        ...
        while t < tmax and steps < max_steps:
            if (steps & 0x07FF) == 0:
                if should_stop is not None and should_stop():
                    break
                if throttle:
                    # ~8 ms of arithmetic, then a real hand-over.  Without
                    # this the solver keeps the interpreter to itself and
                    # the pendulum on screen jerks while it runs.
                    time.sleep(0.001)
            steps += 1
            # ---------------- contact ----------------
            F1 = [0.0, -m1 * self.g]
...
class Run:
    ...
    def retrigger(self, t_div, v_div, mem):
        ...
        if not nxt:
            return False
        self.t0_trig = nxt[0]
        # NB build=True: Run/Auto only ever gets here with Roll off, and
        # this used to reference an undefined name, which threw out of the
        # animation callback and killed the frame loop for good - the
        # instrument and the bench both froze the moment the first sweep
        # ran off the end of its record.
        self.acquire(t_div, v_div, mem, build=True)
        return True
...
class CoilPanel(tk.Canvas):
    ...
    def redraw(self):
        ...
        self.create_text(120, 62, anchor="w",
                         text="코일 통전" if lit else
                              ("스위치는 ON이지만\n12 V 어댑터가 빠져 있음" if on else "코일 꺼짐"),
                         fill="#0d2338" if lit else "#7a2020",
                         font=("TkDefaultFont", 8, "bold"))
        self.create_text(120, 92, anchor="w",
                         text="레버를 내리면 공이 놓입니다"
                         if lit else "레버를 올리면 공을 잡습니다",
...
class WiringPage(tk.Canvas):
    ...
    def redraw(self):
        ...
        self.create_line(140, 190, 146, 210, 140, 220, fill="#26292d",
                         width=1, smooth=True)
        self.create_text(136, 248, text="전자석 홀더  11",
        ...
        self.create_text(136, 376, text="광게이트  10", fill="#c9ccd1",
        ...
        self.create_text(138, 450, text="어댑터 220 V / 12 V   7",
                         fill="#c9ccd1", font=("TkDefaultFont", 8))
        ...
        self.create_text(20, 22, anchor="w", fill="#aeb4ba",
                         font=("TkDefaultFont", 8),
                         text="단자를 클릭한 뒤 연결할 단자를 클릭하세요. "
                              "리드를 클릭하면 빠지고, 우클릭하면 취소됩니다. "
                              "단자 하나에 리드 하나입니다.")
...
class App:
    ...
    def __init__(self, root):
        ...
        self.fps = 0.0
        self._frame_i = 0
        # the instrument arrives switched off, as it does on the bench
        self.power = False
        self._boot_t = None       # wall clock at which the LCD comes up
        # nothing on the screen is locked to the experiment until Auto
        # Setup has been pressed once for the leads that are in now
        self.auto_done = False
        self._junk_rng = np.random.default_rng(7)
        self._junk_ph = 0.0
        self._t0_wall = time.perf_counter()
        self._geo_job = None      # pending (coalesced) scene rebuild
        self.bg_i = self.bg_a = None
        self.v_off = 0.0
        self.v_div2 = 1.0
        self.trig_lvl = 1.0
        ...
        self.nb.add(self.page_bench, text="  실험대와 오실로스코프  ")
        self.nb.add(self.page_wire, text="  결선  ")
        ...
        self._build_plot()
        self._sync()
        if _fast is not None and _fast.HAVE_NUMBA:
            threading.Thread(target=_fast.warm_up, daemon=True).start()
        self._tick()
    ...
    def _build_widgets(self):
        ...
        ttk.Label(left, text="행어에 매단 공",
                  font=("", 10, "bold")).grid(row=r, column=0, sticky="w")
        r += 1
        self.cb = []
        for i in (0, 1):
            ttk.Label(left, text="진자 %d" % (i + 1)).grid(
        ...
        ttk.Checkbutton(left, text="진자 2 사용   (해제 = Part A / B)",
        ...
        ttk.Separator(left, orient="horizontal").grid(
            row=r, column=0, sticky="ew", pady=6)
        r += 1
        ttk.Button(left, text="결선 페이지 열기",
        ...
        ttk.Separator(left, orient="horizontal").grid(
            row=r, column=0, sticky="ew", pady=6)
        r += 1
        self.lbl_sel = ttk.Label(left, text="휠 대상:  -  (먼저 다이얼을 클릭하세요)",
                                 foreground="#a03000")
        ...
        r += 1
        self.lbl_cursor = ttk.Label(left, text="커서 :  -")
        self.lbl_cursor.grid(row=r, column=0, sticky="w", pady=(4, 0))
        r += 1
        ttk.Button(left, text="화면 PNG 저장", command=self.save_png).grid(
            row=r, column=0, sticky="ew", pady=(6, 2))
        r += 1
        ttk.Label(left, wraplength=260, foreground="#555", justify="left",
                  text=("오실로스코프의 모든 설정은 기기 위에 있습니다. "
                        "다이얼을 클릭해 선택한 뒤 마우스 휠을 굴리고, "
                        "노브 중앙을 누르면 미세 스텝입니다. "
                        "Acquire는 획득 메모리를 순환시킵니다. "
                        "Single은 한 발이고 Run/Stop은 래치라, "
                        "Run 상태에서는 릴리스할 때마다 다시 획득합니다. "
                        "진자가 이미 흔들리는 중에 스코프를 켜도 됩니다. "
                        ""
                        "다음 펄스를 기다립니다.\n"
                        ""
                        "원하는 각도까지 공을 당기고 - 각도판에서 끌거나 "
                        "슬라이더를 쓰면 됩니다 - "
                        "전자석이 따라옵니다.  코일이 살아 있으면 공이 붙고, COIL 레버를 내리면 놓입니다.\n\n"
                        "그라티큘 14 x 8, 보조 눈금 0.1 div. 확대 기능은 없습니다. "
                        "에지의 반진폭을 중앙선에 맞추고 M Pos를 7자리까지 읽으세요. "
                        "첫 번째 펄스는 절대 쓰지 마세요."
    ...
    def _collect_animated(self):
        ...
        # Three groups, because a frame of the animation only ever changes
        # what is on the glass and what is on the bench.  The rest of the
        # front panel moves when a hand touches it and not otherwise, so
        # it is pointless to re-upload those pixels sixty times a second.
        self.anim_scr = [self.trace, self.dots, self.centre,
                         self.trig, self.trig_lv,
                         self.txt_hi, self.txt_sa, self.txt_lo,
                         self.scr_off, self.txt_boot]
        # the knob pointers move whenever a dial is turned, so they belong
        # here rather than in the cached panel: otherwise one wheel click
        # would cost a whole canvas redraw
        self.anim_pan = (list(self.knobs.values())
                         + [self.pwr_body, self.pwr_ring, self.pwr_stem,
                            self.sel_ring])
        self.anim_a = (list(self.art_wire) + list(self.art_ball)
                       + [self.art_mline, self.art_mball, self.art_mag,
                          ...
                          self.art_grip]
                       + self.art_screws)
        self.anim = self.anim_scr + self.anim_pan + self.anim_a
        for a in self.anim:
            a.set_animated(True)
        self.background = None

    def _glass_bbox(self):
        """The LCD rectangle in display pixels, padded by a couple."""
        from matplotlib.transforms import Bbox
        tr = self.ax_i.transData
        (x0, y0) = tr.transform((self.SX0, self.SY0))
        (x1, y1) = tr.transform((self.SX0 + self.SW, self.SY0 + self.SH))
        return Bbox([[x0 - 2, y0 - 2], [x1 + 2, y1 + 2]])

    def _on_draw(self, _ev=None):
        # one cached bitmap per axes, so repainting the instrument never
        # has to touch the bench pixels and the other way round, plus a
        # third for the glass on its own: that is the only part of the
        # panel a frame of the animation ever changes
        self.bb_scr = self._glass_bbox()
        self.bg_i = self.canvas.copy_from_bbox(self.ax_i.bbox)
        self.bg_a = self.canvas.copy_from_bbox(self.ax_a.bbox)
        self.bg_scr = self.canvas.copy_from_bbox(self.bb_scr)
        self.background = True
        # Save PNG renders the figure at another dpi, and the regions it
        # would leave behind do not match the screen.  Remember what they
        # were taken at so _blit can tell.
        self._bg_dpi = self.fig.dpi
        for a in getattr(self, "anim", []):
            a.axes.draw_artist(a)

    def _blit(self, only=None):
        """
        only=None      the whole figure
            'bench'    the apparatus half
            'scope'    the whole front panel
            'screen'   the LCD alone - what one animation frame needs
        """
        if (not hasattr(self, "anim") or self.background is None
                or self.bg_i is None or self.bg_a is None
                or getattr(self, "_bg_dpi", None) != self.fig.dpi):
            self.canvas.draw()
            return
        cv, ax_i, ax_a = self.canvas, self.ax_i, self.ax_a
        if only == "screen":
            cv.restore_region(self.bg_scr)
            for a in self.anim_scr:
                ax_i.draw_artist(a)
            cv.blit(self.bb_scr)
            return
        do_i = only != "bench"          # the instrument lives in ax_i
        do_a = only != "scope"          # the bench lives in ax_a
        if do_i:
            cv.restore_region(self.bg_i)
            for a in self.anim_scr:
                ax_i.draw_artist(a)
            for a in self.anim_pan:
                ax_i.draw_artist(a)
        if do_a:
            cv.restore_region(self.bg_a)
            for a in self.anim_a:
                ax_a.draw_artist(a)
        if do_i:
            cv.blit(ax_i.bbox)
        if do_a:
    ...
    def build_instrument(self):
        ...
        self.sel = None
        # every knob is knurled with 18 short strokes.  Drawn as separate
        # Line2D artists that is 144 artists the renderer has to walk on
        # each full redraw; collected here they cost one.
        knurl = []
        ...
        def knob(key, fx, fy, fr, label="", ring=None):
            ...
            for k in range(18):
                aa = k * math.pi / 9.0
                knurl.append([(x + r * 0.80 * math.cos(aa),
        ...
        # item 14 : the power soft key, bottom left of the case exactly
        # where it sits on the SDS 1202X-E.  It is a real switch here, and
        # the instrument starts switched OFF.
        px, py, pr = X(0.1103), Y(0.1270), 0.0153 * W
        self.pwr_body = Circle((px, py), pr, facecolor="#c2c2bf",
                               edgecolor="#9a9a97", lw=1.2, zorder=4)
        ax.add_patch(self.pwr_body)
        self.pwr_ring = Circle((px, py), 0.0070 * W, facecolor="none",
                               edgecolor="#55565a", lw=1.2, zorder=5)
        ax.add_patch(self.pwr_ring)
        (self.pwr_stem,) = ax.plot([px, px], [py, py + 0.011 * H],
                                   color="#55565a", lw=1.4, zorder=6)
        self.hits.append(("power", px - pr, py - pr, (2 * pr, 2 * pr), "btn"))
        ax.text(px, py - Y(0.052), "Power", ha="center", fontsize=4.6,
                color="#3a3b3e", zorder=5)

        # the dark LCD, laid over the graticule while the mains switch is
        # off so that switching on and off costs a blit, not a redraw
        self.scr_off = Rectangle((self.SX0, self.SY0), self.SW, self.SH,
                                 facecolor="#0a0b0c", edgecolor="none",
                                 zorder=20)
        ax.add_patch(self.scr_off)
        self.txt_boot = ax.text(self.SX0 + self.SW / 2,
                                self.SY0 + self.SH / 2, "", color="#2f6fd0",
                                fontsize=12, fontweight="bold", ha="center",
                                va="center", zorder=21)

        ax.add_collection(LineCollection(knurl, colors="#c4c4c1",
        ...
        ax.text(X(0.985), Y(0.055), "다이얼 클릭 후 마우스 휠",
    ...
    def _select(self, key):
        ...
        self.lbl_sel.config(text="휠 대상:  %s%s"
    ...
    def _on_click(self, ev):
        ...
        if h is None:
            return
        key, x, y, kind = h
        if not self.power and key != "power":
            return                       # the front panel is dead
        if kind == "btn":
            self._press(key)
            return
    ...
    def _on_drag(self, ev):
        ...
        ang = math.degrees(math.atan2(-(ev.xdata - b.xh[0]),
                                      max(-ev.ydata, 1e-9)))
        # setting the variable drives the slider, whose command is
        # _on_geo("phi"): that already syncs and repaints the bench
    ...
    def _turn(self, key, s):
        if not self.power:                # a dead instrument does nothing
            return
        line = self.knobs.get(key)        # the pointer is an animated
        if line is not None:              # artist, so this costs a blit
            line._ang += s * 0.28
    ...
    def set_power(self, on):
        """
        The mains soft key at the bottom left of the case.

        Switching off blanks the LCD and deadens the whole front panel;
        the bench is a separate machine and carries on swinging, exactly
        as it would if you pulled the plug on the oscilloscope alone.
        Switching on runs the short boot the instrument really does, then
        comes up in its power-on state: Auto trigger, running.
        """
        self.power = bool(on)
        self.sel = None
        self.sel_ring.set_visible(False)
        self.lbl_sel.config(text="휠 대상:  -  (먼저 다이얼을 클릭하세요)")
        if self.power:
            self._boot_t = time.perf_counter() + 1.6
            self.frozen = None
            self.run = None
            self.auto_done = False        # it boots set up for nothing
            self.trig_mode = "Auto"
            self.cont = True
            self.armed = True
            self._warn(self._wire_hint())
        else:
            self._boot_t = None
            self.run = None
            self.frozen = None
    ...
    def _power_face(self):
        """Light or extinguish the key itself."""
        lit = self.power
        self.pwr_body.set_facecolor("#cdeeb0" if lit else "#c2c2bf")
        self.pwr_body.set_edgecolor("#9ab98a" if lit else "#9a9a97")
        col = "#3a5a2a" if lit else "#55565a"
        self.pwr_ring.set_edgecolor(col)
        self.pwr_stem.set_color(col)

    def _press(self, key):
        if key == "power":
            self.set_power(not self.power)
            return
        if not self.power:
            return
        if key == "single":
            ...
            # Auto Setup: the instrument looks at what is actually on the
            # input, picks a timebase and a scale for it and locks the
            # sweep to it.  Until this has been done once for the leads
            # that are in now, nothing on the glass stands still.
            self.auto_done = True
            ...
            # factory settings, which includes forgetting the setup
            self.auto_done = False
            self.run = None
            self.frozen = None
            self.t_div, self.v_div, self.v_off = 0.5, 2.0, 0.0
    ...
    def _on_geo(self, which=None):
        """
        A dragged Tk scale fires one of these per pixel of travel, and a
        full scene rebuild plus a canvas redraw is far too heavy to do at
        that rate.  Pulling the ball aside moves nothing static at all, so
        it is answered straight away with a blit; everything else is
        coalesced and applied once the slider settles.
        """
        self._sync()
        if which == "phi":
            # only the holder and the dashed ball move, and both of those
            # are animated artists
            self.update_scene()
            self._blit("bench")
            return
        if self._geo_job is not None:
            try:
                ...
                pass
        self._geo_job = self.root.after(80, self._apply_geo)
        ...
        if on:
            ...
            if not self.wire_state["powered"]:
                self.lbl_warn.config(
                    text="박스에 12 V가 없습니다. 결선 페이지에서 어댑터를 DC 12V에, 홀더 코일을 MAGNET에 연결하세요.")
            else:
                self.lbl_warn.config(text="")
        else:
            if was_held:
    ...
    def wiring_changed(self):
        ...
        self.coil_panel.redraw()
        self._sync()
        # the leads have moved, so whatever the instrument was set up for
        # is no longer what is on its input: it has to be Auto Set again,
        # and anything it had captured belongs to the old connection
        self.auto_done = False
        self.run = None
        self.frozen = None
        self._warn(self._wire_hint())
        self.draw_scope()        # the old picture belonged to the old leads
        self.build_scene()
        self.background = None
        self.canvas.draw_idle()
    ...
    def release(self):
        ...
        two = self.bench.active2
        tail = 300.0            # follow the swing until it is at rest
        # only as much as the background thread needs to catch up: the
        # first stage it delivers is 12 s, and computing 40 s here was a
        # visible freeze on the click that lets the ball go.  Two balls
        # need long enough for the first gate pulse to CLOSE, otherwise
        # the release is thrown away below as "no trigger" and the bench
        # never starts at all.
        head = 2.0 if two else 12.0
        spare = self.bench.clone()
        run = Run(self.bench, self.t_div, self.v_div,
        ...
        if tail > head:
            ...
            def _tail_worker(b=spare, box=box, tm=tail, src=run.source,
                             gen=mygen):
                # deliver in stages, and give up as soon as the bench has
                # moved on, so an abandoned solve stops eating the
                # interpreter instead of dragging on the next release
                try:
                    for stage in (12.0, 60.0, tm):
                        if self._gen != gen:
                            return
                        r = b.simulate(tmax=stage, throttle=True,
                                       should_stop=lambda: self._gen != gen)
                        if self._gen != gen:
                            return
                        box["built"] = Run.derive(r, src)
                        box["res"] = r
        ...
        fault = self.wiring.fault()
        run.fault = fault
        # A floating input is drawn live by _free_run - there is no point
        # building a record out of mains hum - and nothing is captured at
        # all until Auto Setup has been pressed for the leads that are in.
        usable = (st["source"] is not None and st["channel"] == "ch1"
                  and st["ground"] and self.auto_done)
        self.run = run if (self.armed and usable and ok) else None
        if self.armed and st["live"] and not ok:
            self.lbl_warn.config(
                text="트리거 레벨이 신호 밖에 있어 스코프가 걸리지 않습니다. LEVEL 노브를 펄스 높이 안쪽으로 되돌리세요.")
        msg = {"open": "CH1에 아무것도 들어오지 않습니다. 결선 페이지에서 신호원 리드와 CH1/2 리드를 확인하세요.",
               "wrongch": "박스는 CH2에 물려 있는데 화면은 CH1을 보고 있습니다.",
               "float": "접지 리드가 빠져 입력이 떠 있습니다. 지금 보이는 것은 케이블이 주워 온 상용전원 유도이지 실험 신호가 아닙니다."}.get(fault)
        if msg is None and not self.auto_done:
            msg = ("스코프가 아직 아무것에도 동기되지 않았습니다. Auto Setup을 "
                   "한 번 눌러 시간축과 레벨을 잡으세요.")
        if msg:
    ...
    def _extend(self):
        """
        Ask for another stretch of trajectory, carrying on from the last
        frame of the one we have.  Without this the bench simply stopped
        when the pre-computed run ended - a minute of swinging with two
        pendulums, which is exactly when it looked like a freeze.
        """
        # NB the guard tests an explicit flag.  It used to test the box
        # itself, and an empty dict is falsy, so this fired on every
        # single frame: each call bumped the generation counter, which
        # told the solve started one frame earlier to give up.  Nothing
        # ever finished and the pendulum stopped dead at the end of the
        # pre-computed stretch.
        if self._played is None or self._pending.get("busy"):
            return
        self._gen = getattr(self, "_gen", 0) + 1
        r = self._played.res
        if not len(r["t"]):
            return
        f = r["p"][-1]
        a = r["p"][-2] if len(r["p"]) > 1 else f
        dt = max(r["t"][-1] - r["t"][-2], 1e-9) if len(r["t"]) > 1 else 1e-3
        state = (((f[0], f[1]), ((f[0] - a[0]) / dt, (f[1] - a[1]) / dt),
                  (f[2], f[3]), ((f[2] - a[2]) / dt, (f[3] - a[3]) / dt),
                  f[4], f[5]))
        spare = self.bench.clone()
        box = self._pending = {"busy": True}
        t_end = float(r["t"][-1])
        ...
        def _worker(b=spare, box=box, st=state, tm=span, t0=t_end,
                    src=self._played.source, gen=gen):
            try:
                if self._gen != gen:
                    return
                res = b.simulate(tmax=tm, init=st, throttle=True,
                                 should_stop=lambda: self._gen != gen)
                if self._gen != gen or not len(res["t"]):
                    box["append"] = None
                    return
                res["t"] = res["t"] + t0          # continue the clock
                for k in ("gate", "contact", "segments"):
                    if res.get(k):
                        res[k] = [(a2 + t0, b2 + t0) for a2, b2 in res[k]]
                box["append"] = res
    ...
    def _splice(self, res):
        """Join a fresh stretch onto the end of the one being played."""
        p = self._played
        old = p.res
        joined = dict(old)
        joined["t"] = np.concatenate([old["t"], res["t"]])
        joined["p"] = np.concatenate([old["p"], res["p"]])
        for k in ("gate", "contact", "segments"):
            joined[k] = list(old.get(k) or []) + list(res.get(k) or [])
        p._build(joined)
    ...
    def _absorb_tail(self):
        ...
        if not box:
            return False
        if "append" in box:                    # a spliced continuation
            app = box.pop("append")
            if app is not None and len(app["t"]):
                self._splice(app)
                return True
            return False
        # a whole fresh trajectory.  The worker writes "built" first and
        # "res" second, so keying off "res" makes sure a stage is only
        # taken once both halves of it are there.
        res = box.get("res")
        if res is None or not len(res["t"]) or self._played is None:
            return False
        built = box.pop("built", None)
        box.pop("res", None)
    ...
    def arm_now(self):
        ...
        if p is None or not p.pulses or not self.wire_state["live"]:
            return False
        if not self.auto_done:               # not set up for these leads
            return False
    ...
    def _on_motion(self, ev):
        if (ev.inaxes is self.ax_i and ev.xdata is not None
            ...
            self.lbl_cursor.config(text="커서 :  t = %s   V = %+.3f V"
                                        % (eng(t, "s"), v))
        else:
            self.lbl_cursor.config(text="커서 :  -")
    ...
    def build_scene(self):
        ...
        # the configuration caption only changes with the geometry, which
        # rebuilds this scene anyway, so it stays in the cached background
        # instead of being re-laid-out on every single frame
        self.art_cfg = ax.text(-0.400, 0.054, b.config_name(), fontsize=8,
    ...
    def update_scene(self):
        ...
        if self.play:
            ...
            self.art_grip.set_text("공이 붙었습니다  -  각도판에서 각을 읽은 뒤 COIL 레버를 내리세요")
            self.art_grip.set_color("#207020")
        else:
            self.art_grip.set_text("코일에 전류가 흐르지 않습니다 - 공은 그냥 매달려 있습니다.  "
                                   "결선 페이지를 확인하세요.")
    ...
    def _blank_screen(self, boot=""):
        """Dark LCD: nothing on the glass but the boot splash, if any."""
        self.scr_off.set_visible(True)
        self.trace.set_data([], [])
        self.dots.set_data([], [])
        self.centre.set_data([], [])
        self.trig.set_data([], [])
        self.trig_lv.set_data([], [])
        self.txt_hi.set_text("")
        self.txt_sa.set_text("")
        self.txt_lo.set_text("")
        self.txt_boot.set_text(boot)
        self.lbl_acq.config(text="")
        self.lbl_mpos.config(text="M Pos = ----")

    def _float_now(self):
        """Is CH1 fed by a lead whose ground return is not connected?"""
        st = getattr(self, "wire_state", None) or {}
        return (st.get("source") is not None and st.get("channel") == "ch1"
                and not st.get("ground"))

    def _no_signal_now(self):
        """Is there any path at all from the bench to CH1 and back?"""
        st = getattr(self, "wire_state", None) or {}
        return not (st.get("source") is not None
                    and st.get("channel") == "ch1" and st.get("ground"))

    def _warn(self, text):
        if self.lbl_warn.cget("text") != text:
            self.lbl_warn.config(text=text)

    def _wire_hint(self):
        """What is wrong with the connection as it stands, in one line."""
        f = self.wiring.fault()
        if f == "open":
            return ("CH1에 아무것도 들어오지 않습니다. 결선 페이지에서 신호원 "
                    "리드와 CH1/2 리드를 확인하세요.")
        if f == "wrongch":
            return "박스는 CH2에 물려 있는데 화면은 CH1을 보고 있습니다."
        if f == "float":
            return ("접지 리드가 빠져 입력이 떠 있습니다. 지금 화면에 보이는 "
                    "것은 케이블이 주워 온 상용전원 유도와 드리프트이지 실험 "
                    "신호가 아닙니다. 결선 페이지에서 박스의 GND를 스코프 "
                    "접지에 연결하세요.")
        if not self.auto_done:
            return ("스코프가 아직 아무것에도 동기되지 않았습니다. 리드를 "
                    "연결한 뒤에는 Auto Setup을 한 번 눌러 시간축과 레벨을 "
                    "잡으세요.")
        return ""

    def _free_run(self, floating):
        """
        The screen when the sweep is not locked to the experiment.

        floating=True
            The ground lead is out.  A 1 M ohm input on the far end of a
            metre of wire with no return path is an aerial, not a probe:
            the mains and its harmonics arrive at several volts, the
            baseline wanders about, and the sweep triggers on the hum
            instead of on the photogate.  None of that comes from the
            bench, so it is there whether the pendulum swings or hangs
            dead still - which is exactly how you recognise the fault.

        floating=False
            The leads are right but Auto Setup has not been pressed since
            they were changed.  The timebase and the level are whatever
            they happened to be, nothing is synchronised, and each sweep
            starts somewhere else, so the pulses slide across the glass
            and never stand still.
        """
        span = self.NX * self.t_div
        n = int(min(max(self.npix, 200), 700))
        rg = self._junk_rng
        now = time.perf_counter() - self._t0_wall
        xc = (np.arange(n) + 0.5) / n * self.NX
        ys = np.empty(2 * n, dtype=np.float32)

        if floating:
            # enough samples per mains period that a slow sweep really
            # does come out as the solid band a real one shows
            k = int(min(max(round(2400.0 * span / n), 1), 30))
            m = n * k
            t = now + rg.random() * 0.02 + np.arange(m) * (span / m)
            ph = self._junk_ph
            self._junk_ph += 0.37
            v = (4.6 * np.sin(2 * np.pi * 50.0 * t + ph)
                 + 1.8 * np.sin(2 * np.pi * 150.0 * t + 2.1 * ph + 0.7)
                 + 0.8 * np.sin(2 * np.pi * 250.0 * t + 1.3)
                 + 2.6 * np.sin(2 * np.pi * 0.23 * t + 0.4 * ph)
                 + 0.35 * np.sin(2 * np.pi * 8.7e3 * t))
            v += 0.55 * rg.standard_normal(m)
            if k > 1:
                vv = v.reshape(n, k)
                ys[0::2], ys[1::2] = vv.min(axis=1), vv.max(axis=1)
            else:
                ys[0::2] = ys[1::2] = v
        else:
            lo = np.full(n, V_LOW, dtype=np.float32)
            hi = np.full(n, V_LOW, dtype=np.float32)
            p = self._played
            # an open or misplugged input has nothing on it but its own
            # noise floor, however hard the bench is working
            if (not self._no_signal_now()
                    and p is not None and getattr(p, "pulses", None)):
                v_hi = getattr(p, "v_hi", 5.0)
                a0 = rg.random() * max(self.play_t, span)
                edges = np.linspace(a0, a0 + span, n + 1)
                for a, b in p.pulses:
                    if b < edges[0] or a > edges[-1]:
                        continue
                    i = max(int(np.searchsorted(edges, a) - 1), 0)
                    j = min(int(np.searchsorted(edges, b)), n)
                    if j > i:
                        hi[i:j] = v_hi
            nz = 0.02 * rg.standard_normal(n)
            ys[0::2], ys[1::2] = lo + nz, hi + nz
        ...
        if floating and abs(self.trig_lvl) < 5.0:
            state, col = "Trig'd", "#ffd23a"   # it fires, on the hum
        else:
        ...
        self.txt_lo.set_text("M %s   CH1 = %g V   M Pos = %.6f s"
                             % (tdiv_label(self.t_div), self.v_div,
                                self.delay))
        # nothing has been captured, so the sidebar shows what the front
        # panel would acquire with, not what it did
        mem = MEM_DEPTHS[self.mem_i]
        fs = snap_fs(mem / span)
        nn = int(min(mem, max(span * fs, 8)))
        _, gy, gw, gh = self._grat()
        self.txt_sa.set_position((self.SX0 + gw + 4, gy + gh - 6))
        self.txt_sa.set_text("Sa %s\nCurr %s" % (fs_label(fs),
                                                 mem_label(nn)))
        self.lbl_acq.config(text="획득 조건 %s/div\n%s , %s"
                                 % (tdiv_label(self.t_div), fs_label(fs),
                                    mem_label(nn)))
        self._warn(self._wire_hint())
    ...
    def draw_scope(self):
        ...
        if not self.power:
            self._blank_screen()
            return
        if self._boot_t is not None:
            self._blank_screen("SIGLENT")
            return
        self.scr_off.set_visible(False)
        self.txt_boot.set_text("")
        if self.frozen is not None:
            ...
            self.lbl_mpos.config(text="M Pos = %.6f s" % self.delay)
            return
        # a floating input is a mess whatever the sweep is doing, and an
        # instrument that has not been Auto Set since the leads changed
        # is not locked to anything either
        if self.cont or self.armed:
            if self._float_now():
                self._free_run(True)
                return
            if self.cont and (not self.auto_done or self._no_signal_now()):
        ...
        if run is not None and run.rec is not None:
            ...
            self.lbl_acq.config(
                ...
                + ("\n%.0f fps  (트레이스 %d점)" % (self.fps, self.npix)
                   if self.play else ""))
            if run.feature and run.feature < 10 * dts:
                self.lbl_warn.config(
                    text=("측정하려는 특징이 표본 %.2f개 폭밖에 되지 않습니다. 지금 보이는 것은 몇 점을 sin(x)/x로 재구성한 것입니다. 시간축을 빠르게 하거나 획득 메모리를 깊게 한 뒤 Single을 누르세요."
                          % (run.feature / dts)))
        else:
            self.txt_sa.set_text("")
            self.lbl_acq.config(text="")
    ...
    def _tick(self):
        ...
        if self.play and self._played is not None:
            ...
            # never run past what has been computed: the clock waits for
            # the next chunk rather than freezing and then teleporting
            self.play_t = min(self.play_t + dt, t_end)
            if self.play_t > t_end - 6.0:
                self._extend()                     # keep the bench alive
            self.update_scene()
            moving = True
        ...
        booting = False
        if self._boot_t is not None:
            if t_start >= self._boot_t:
                self._boot_t = None
                if self.play:
                    self.arm_now()
            else:
                booting = True
        if (self.power and not booting and self.cont
                and self.run is not None and not self.roll):
            r = self.run
            if (r.rec is not None and self.play_t - r.t0_trig
                ...
                if r.retrigger(self.t_div, self.v_div,
                    ...
                    # nothing static changed: the cached panel still holds

        # a floating input never settles, so the screen has to keep being
        # repainted even when the sweep itself is only armed.  An armed
        # Single has to keep being repainted too, or the trace it is
        # waiting for never appears.
        junk = (self.power and self.frozen is None
                and (self.cont or self.armed) and self._float_now())
        scope_live = self.power and (booting or moving or self.cont
                                     or self.armed or junk)
        if moving or scope_live:
            self._frame_i += 1
            # The only pixels a frame can change are the ones on the glass
            # and the ones on the bench.  The rest of the front panel moves
            # when a hand touches it, so it is never re-uploaded here.  The
            # trace also changes far more slowly than the pendulum does, so
            # it is rebuilt every third frame while the bench is running.
            if scope_live and (self._frame_i % 3 == 0 or not moving):
                self.draw_scope()
                self._blit("screen")
            if moving:
                self._blit("bench")
            ...
            if self.render_ms > 26.0 and self.npix > 300:
                ...
                self.npix *= 2
            # aim at a steady ~60 Hz and let a slow machine drop frames
            # rather than queue them up
            delay = int(min(max(16.0 - self.render_ms, 1.0), 60.0))
        else:
            delay = 50                   # nothing is moving: idle cheaply
