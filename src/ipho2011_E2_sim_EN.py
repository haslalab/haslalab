...
class Rig:
    ...
    def mount_on_loop(self, x=None):
        ...
        if not self.loop_present:
            return self._msg('Hang the thread loop on the pin first.')
        if self.mode == "pendulum":
            return self._msg('The cylinder is already hanging on the pin.')
        self.mode = "balance"
        ...
        self._fall_v = 0.0
        self._fall_drop = 0.0
        return True, 'Cylinder placed in the loop. Drag the loop to balance it.'

    def mount_on_pin(self, hole_index=0):
        """Hang the cylinder on the pin through one of the drilled holes."""
        if self.mode == "balance":
            return self._msg('Take the thread loop off first.')
        if not (0 <= hole_index < len(self.truth.holes)):
            return self._msg('No such hole.')
        self.mode = "pendulum"
        ...
        self.released = False
        self.fallen = False
        return True, 'Cylinder hung on the pin.'
    ...
    def enter_part_b(self):
        ...
        self.sel_hole = None
        self.hole_index = None
        return True, 'Cylinder placed on the bench.'

    def select_hole(self, i):
        if not (0 <= i < len(self.truth.holes)):
            return False, ""
        self.sel_hole = i
        return True, 'Hole #%d selected. Now click the pin.' % (i + 1)

    def connect_selected_to_pin(self):
        if self.sel_hole is None:
            return self._msg('Click one of the holes in the cylinder first.')
    ...
    def unmount_from_pin(self):
        ...
        self.theta = self.omega = 0.0
        self.released = False
        return True, 'Cylinder taken off the pin.'
    ...
    def take_cylinder_off(self):
        ...
        self.fallen = False
        self.carried = False
        return True, 'Cylinder taken off.'

    def take_thread_off(self):
        if self.mode == "balance":
            self.take_cylinder_off()
        self.loop_present = False
        return True, 'Thread loop removed.'
    ...
    def catch(self):
        ...
        self.hold(True)
        return True, 'Caught it with your finger.'
    ...
    def hold(self, on):
        ...
        if self.held:
            self.slipping = False
            self._slide_v = 0.0
        return True, ('Holding the cylinder steady.' if self.held else
                      'Let go.')
    ...
    def pick_up_cylinder(self):
        ...
        if self.mode != "balance" or not self.fallen:
            return False, ""
        self.carried = True
        return True, 'Cylinder in hand. Bring it to the loop and click.'
    ...
    def insert_into_loop(self):
        ...
        self.hold(True)
        return ok, 'Cylinder back in the loop.'
    ...
    def release(self):
        if self.mode != "pendulum":
            return self._msg('Hang the cylinder on the pin first.')
        self.released = True
        return True, ""
    ...
    def status_line(self):
        ...
        if r["mode"] == "free":
            if self.sel_hole is None:
                return 'Part B - click a pivot hole in the cylinder.'
            return 'Part B - hole #%d selected. Click the pin.' % (
                         self.sel_hole + 1)
        if r["mode"] == "balance":
            d = {"level": 'level',
                 "bottom_down": 'far end down',
                 "top_down": 'near end down',
                 "carried": 'in hand',
                 "held": 'supported by hand - left click to let go',
                 "fallen": 'slipped out - click it to pick it up'}
            s = 'Balance - ' + d[r["tilt_view"]]
            if r.get("sliding"):
                s += '  (slipping!)'
            return s
        return ('Pendulum - hole #%d, angle %.1f deg'
...
class LabView(ttk.Frame):
    ...
    def __init__(self, master, app, part):
        ...
        self.hover_xy = (0, 0)
        self.tip_text = 'wheel on tube: loop - on ruler: turn - click tube: let go - elsewhere: zoom' \
            if part == "A" else \
            'wheel on ruler: turn - elsewhere: zoom - right click: off the pin - space: stopwatch'
    ...
    def build_panel(self):
        p = self.panel
        head = 'Part A - centre of mass & length' \
            if self.part == "A" else 'Part B - period measurement'
        ttk.Label(p, text=head, font=("TkDefaultFont", 11, "bold")).pack(
            pady=(9, 3))

        hint = '- wheel over the tube: slide the loop 2 mm\n  (Shift+wheel = 0.2 mm fine steps)\n- the finger keeps holding the tube until\n  you click the tube itself\n- click while it is slipping to catch it\n  with your finger\n- click a fallen tube to pick it up, then\n  click at the loop to slide it back in\n- drag the ruler, pencil and rubber about\n  (sweep the pencil to mark the tube,\n   the rubber to wipe marks off)\n- the wheel over the ruler turns it about\n  the point under the cursor\n- the wheel anywhere else zooms\n- Ctrl+wheel: always zooms' if self.part == "A" else \
            '- click a pivot hole in the cylinder, then\n  click the pin on the base plate to hang\n  the tube by that hole\n- right click: take the tube off the pin\n- drag a hanging tube aside and let go;\n  any amplitude, a full turn included\n- drag the tube about when it is off\n- drag the ruler, pencil and rubber about\n  (the wheel over the ruler turns it)\n- pencil marks made in Part A are still\n  on the tube here\n- wheel: zoom\n- space bar: start / stop the stopwatch'
        ...
        if self.part == "B":
            ...
            row2 = ttk.Frame(p)
            row2.pack(pady=4)
            ttk.Button(row2, text='START', width=7, takefocus=0,
                       command=self.sw_start).pack(side="left", padx=2)
            ttk.Button(row2, text='STOP', width=7, takefocus=0,
                       command=self.sw_stop).pack(side="left", padx=2)
            ttk.Button(row2, text='RESET', width=7, takefocus=0,
    ...
    def build_actions(self):
        ...
        if self.part == "A":
            ...
            ttk.Button(self.act, text='Re-hang the cylinder',
    ...
    def _sync_hold(self):
        ...
        if b is None or not b.winfo_exists():
            return
        b.config(text='Let go' if self.rig.held
                 else 'Support by hand')
        ...
        if self.part == "A":
            if r.carried:
                if math.hypot(wx, wy - LOOP_DROP) < 0.030:
                    ...
                    self.app.status('Click at the hanging loop.')
                return
...
class App(tk.Tk):
    def __init__(self, seed):
        ...
        self.title('IPhO 2011 Experiment 2 - Mechanical Blackbox (HaslaLab)')
        ...
        top = ttk.Frame(self)
        top.pack(fill="x")
        ttk.Label(top, text='2. Mechanical Blackbox: a cylinder with a ball inside',
        ...
        self.nb.add(self.pa, text='Part A - centre of mass & length')
        self.nb.add(self.pb, text='Part B - periods')
